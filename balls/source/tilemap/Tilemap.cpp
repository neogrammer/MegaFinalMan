	...
	for (int y = 0; y < rows; y++)
	{
		for (int x = 0; x < pitch_; x++)
		{
			...
			else
			{
				...
				tiles.emplace_back(std::make_unique<Tile>(tileset->getTexType(), tRef->getSize(), sf::Vector2f{ (float)(x)*tileset->getTiles().at(num)->frameSize.x, (float)(y)*tileset->getTiles().at(num)->frameSize.y }, tileset->getTiles().at(num)->getTexPosition()));
			}
		}
	}
}
