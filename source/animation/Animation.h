...
class Animation
{
	...
	std::vector<sf::Vector2f> texOffsets;
	std::vector<sf::IntRect> rects;
	sf::Vector2f texStartPos;

public:
	Animation(std::string id_ = "Default", std::string dir_ = "Uni", float frameDelay_ = { 0.13f }, unsigned numFrames_ = 1, sf::Vector2f frameSize_ = { 50.f,50.f }, sf::Vector2f texStartPos_ = { 0.f,0.f }, bool loops_ = true, bool loopWaits_ = false, float loopDelay_ = 0.f, sf::Vector2f texOff_ = { 0.f,0.f });
	~Animation();
	...
	std::vector<sf::Vector2f>& getTexOffsets();
	sf::Vector2f getTexOffset(int index_);

	std::vector<sf::IntRect>& getRects();
	sf::IntRect getRect(int index_);
	bool isLoopWaiting();
	void setIsLoopWaiting(bool cond_);
	int getNumFrames();
